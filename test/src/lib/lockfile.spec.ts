...
describe('lib/lockfile', () => {
	const lockPath = '/tmp/balena-supervisor/services/1234567/updates.lock';
	...
	const mockDir = (
		...
		const items: any = {};
		if (opts.createLock) {
			items[basename(lockPath)] = mock.file({ uid: NOBODY_UID });
		}
	...
	it('should create a lockfile as the `nobody` user at target path', async () => {
		...
		// Verify lockfile UID
		expect((await fs.stat(lockPath)).uid).to.equal(NOBODY_UID);
	});
